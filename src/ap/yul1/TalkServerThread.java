...
public class TalkServerThread extends Thread {
	...
	public void broadCasting(String msg) { // 현재 입장해 있는 친구들 모두에게 메시지 전송하기 구현
		for(TalkServerThread tst:ts.globalList) {
			tst.send(msg);
		}
	}
	public void send(String msg) { //클라이언트에게 말하기 구현 
		try {
			oos.writeObject(msg);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void run() {
		...
		try {
			...
			while(!isStop) {
				...
				ts.jta_log.append(msg+"\n");  // 메시지출력
				ts.jta_log.setCaretPosition // 입력창에 맞춰 스크롤바 자동 이동
